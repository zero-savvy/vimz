...
template Crop(hOrig, wOrig, hNew, wNew) {
	...
	// 		// tmp1 == (i >= hStartNew) && (i < hStartNew + hNew);
	// 		// tmp2 == (j >= wStartNew) && (j < wStartNew + wNew);
	// 		// yes == tmp1 && tmp2;
	// 		tmp [i][j] = 0;
	// 		// tmp [i][j] == yes ? original[i][j] : 0;
	// 	}		
	// }

	// for (var k = 0; k < hNew; k++) {
	// 	mux[k] = Multiplexer(wOrig, hOrig);
	// 	mux[k].inp <== original;
	// 	mux[k].sel <== hStartNew + k;
	// 		// compare_matrix[][] = 
	// }
...
component main {public[hash]}= Crop(72, 128, 48, 64);
